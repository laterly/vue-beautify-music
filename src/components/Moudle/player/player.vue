  ...
  methods: {
    change() {
      let newTime = parseInt(
        document.getElementById("audioPlay").currentTime * 1000
      );
      let per = 100;
    ...
    onChange() {
      let site =
        this.$store.state.player.nowPlaying.newRangeValue /
        100 *
        this.player.nowPlaying.timelength;
      this.$store.dispatch(
        "reviseCurrentTime",
        document.getElementById("audioPlay").currentTime * 1000
      );
    ...
    revisePlay() {
      if (this.$store.state.player.nowPlaying.song_name == "在线音乐") return;
    ...
    getPlayList() {
      ...
      switch (type) {
        case 1:
          let data = store.local.get("localPlayList")
            ? store.local.get("localPlayList")
            : [];
          this.list = data;
          ...
          break;
        case 2:
          let songData = store.local.get("playSongList")
            ? store.local.get("playSongList")
            : [];
          this.list = songData;
